...
const int S_Table[8][4][16] = {
    ...
    {{15,1,8,4,6,11,3,4,9,7,2,13,12,0,5,10},{3,13,4,7,15,2,8,14,12,0,1,10,6,9,11,5},{0,14,7,11,10,4,13,1,5,8,12,6,9,3,2,15},{13,8,10,1,3,15,4,2,11,6,7,12,0,5,14,9}},
...
class DesRoundFunction : public RoundFunction
{
        ...
        *tmp_text ^= *tmp_key;
        ...
        for (int i = 0; i < 8; i++) {
            const auto row_ind = ((*tmp_text & (32u << (6 * i))) >> (6 * i + 4)) |
                                 ((*tmp_text & (1u << (6 * i))) >> (6 * i));
            const auto col_ind = (*tmp_text & (30u << (6 * i))) >> (6 * i + 1);
            tmp_res |= (S_Table[7 - i][row_ind][col_ind]) << (4 * i);
        }
        permutations(reinterpret_cast<uint8_t*>(&tmp_res), 32, P_table, 32, result);
    }
};
